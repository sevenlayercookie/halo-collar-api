"""OAuth 2.0 login and refresh-token support for observed Halo mobile clients."""
...
class OAuthClientProfile:
    """Observed metadata that must stay paired with a mobile OAuth session."""
...
class HaloOAuth:
    """Authenticate with one observed Halo mobile OAuth client."""
    ...
    def begin_login(self) -> AuthorizationFlow:
        # The captured iOS client uses a 32-byte verifier (43 base64url chars).
        # Although PKCE permits longer values, match Halo's observed model exactly.
    ...
    def password_login(self, username: str, password: str) -> TokenSet:
        ...
        if self.profile.client_id != ANDROID_PROFILE.client_id:
            raise ValueError("The observed Halo password grant requires the Android profile.")
...
def _callback_location(capture: str) -> str:
    ...
    if not location.casefold().startswith("haloapp://callback"):
        raise InvalidCallbackError(
            "The captured response Location is not a haloapp://callback URL."
        )
    return location
